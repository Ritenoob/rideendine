...
export class OrdersService {
  ...
  async createOrder(customerId: string, createDto: CreateOrderDto) {
    ...
    try {
      ...
      // 5. Validate delivery zone (if chef has location and radius set)
      let distanceMiles = 0;
      let deliveryFeeCents = 500; // Default $5.00
      ...
      // 6. Calculate totals using CommissionCalculator (with dynamic delivery fee)
      const breakdown = CommissionCalculator.calculate(subtotalCents);
      breakdown.deliveryFeeCents = deliveryFeeCents; // Override with distance-based fee
      breakdown.totalCents = breakdown.subtotalCents + breakdown.taxCents + deliveryFeeCents;
  ...
  async getOrderTrackingFull(orderId: string): Promise<OrderTrackingFullDto> {
    const result = await this.db.query(
      `SELECT 
        ...
        o.created_at, o.payment_confirmed_at, o.accepted_at, o.ready_at,
        o.assigned_at, o.picked_up_at, o.delivered_at,
        c.business_name as chef_business_name, c.address as chef_address,
        c.latitude as chef_latitude, c.longitude as chef_longitude,
        cu.first_name as customer_first_name, cu.last_name as customer_last_name,
        chu.first_name as chef_first_name, chu.last_name as chef_last_name,
       ...
       FROM orders o
       JOIN chefs c ON o.chef_id = c.id
       JOIN users chu ON c.user_id = chu.id
  ...
  /**
   * Create an order for Stripe Checkout (status: PENDING)
   */
  async createOrderForCheckout(params: {
    customerId: string;
    chefId: string;
    items: Array<{
      menuItemId: string;
    ...
    try {
      ...
      // Get chef info
      const chefResult = await client.query(
        'SELECT id, business_name, latitude, longitude FROM chefs WHERE id = $1',
        [params.chefId],
      );
      ...
      const chef = chefResult.rows[0];
      const pickupAddress = `${chef.business_name}, Kitchen`; // Simplified for now

      // Create order
      const orderResult = await client.query(
        `INSERT INTO orders (
          customer_id, chef_id, order_number, status,
          subtotal_cents, tax_cents, delivery_fee_cents, platform_fee_cents, total_cents,
          chef_earning_cents, pickup_address, pickup_latitude, pickup_longitude,
        ...
        [
          ...
          params.totalCents,
          params.subtotalCents - params.platformFeeCents, // Chef earnings
          pickupAddress,
          chef.latitude,
          chef.longitude,
      ...
      for (const item of params.items) {
        ...
        const menuItemResult = await client.query(
          'SELECT name, price_cents FROM menu_items WHERE id = $1',
          [item.menuItemId],
        );
        ...
        const itemTotal = menuItem.price_cents * item.quantity;

        await client.query(
          `INSERT INTO order_items (
            order_id, menu_item_id, menu_item_name, quantity, price_cents, total_cents, notes
          )
          VALUES ($1, $2, $3, $4, $5, $6, $7)`,
          [order.id, item.menuItemId, menuItem.name, item.quantity, menuItem.price_cents, itemTotal, item.notes],
        );
      }
      ...
      await client.query(
        `INSERT INTO order_status_history (order_id, status, created_by)
         VALUES ($1, $2, $3)`,
        [order.id, OrderStatus.PENDING, params.customerId],
      );

      await client.query('COMMIT');

      this.logger.log(`Order created for checkout: ${orderNumber} (${order.id})`);
   ...
   */
  async updateCheckoutSession(orderId: string, sessionId: string) {
    await this.db.query(
    ...
    this.logger.log(`Updated order ${orderId} with checkout session ${sessionId}`);
  }
  ...
  async handleCheckoutCompleted(
    ...
    try {
      ...
      await client.query(
        `UPDATE orders 
         SET status = $1, 
             ...
             updated_at = NOW()
         WHERE id = $4`,
        [OrderStatus.PAYMENT_CONFIRMED, sessionId, paymentIntentId, orderId],
      );
      ...
      if (paymentIntentId) {
        ...
        if (orderResult.rows.length > 0) {
          await client.query(
            ...
            [orderId, paymentIntentId, orderResult.rows[0].total_cents, 'succeeded'],
          );
        }
      }
      ...
      await client.query(
        `INSERT INTO order_status_history (order_id, status, notes)
         VALUES ($1, $2, $3)`,
        [orderId, OrderStatus.PAYMENT_CONFIRMED, 'Payment completed via Stripe Checkout'],
      );
  ...
  /**
   * Dispatch order to Mealbridge
   */
  async dispatchToMealbridge(orderId: string) {
    // This is a placeholder - actual implementation would call IntegrationsService
    this.logger.log(`Dispatching order ${orderId} to Mealbridge (not yet implemented)`);
    
    // TODO: Implement actual Mealbridge dispatch
    // const result = await this.integrationsService.dispatchToMealbridge(orderId);
    // return result;
  }

  /**
   * Get public order tracking data (no auth required, redacted info)
   */
  async getOrderTracking(orderId: string): Promise<OrderTrackingDto> {
    const result = await this.db.query(
      `SELECT o.id, o.order_number, o.status, o.estimated_delivery_time, o.created_at,
              o.delivery_latitude, o.delivery_longitude,
              d.current_latitude as driver_latitude, d.current_longitude as driver_longitude,
              d.last_location_update
       FROM orders o
    ...
    if (order.driver_latitude && order.driver_longitude && order.delivery_latitude && order.delivery_longitude) {
      ...
      // Estimate 25 mph average speed
      etaMinutes = Math.ceil((distance / 25) * 60);
    }

    // Generate driver status message based on order status
    ...
    switch (order.status) {
      ...
      case OrderStatus.PREPARING:
        driverStatus = 'Your order is being prepared';
        break;
    ...
    return {
      ...
      estimatedDeliveryTime: order.estimated_delivery_time,
      driverStatus,
      pickupLabel: 'Local chef kitchen', // NEVER reveal actual chef address
      createdAt: order.created_at,
    };
  }
