...
import { Button, Input, Card, TipSelector } from '@/components/ui';
...
export default function CheckoutScreen() {
  ...
  const { initializePayment, processPayment } = useStripePayment();
  const {
    chef,
    items,
    tip,
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      // 1. Get coordinates if not available
      ...
      const initialized = await initializePayment(order.id);
      if (!initialized) {
      ...
      if (!result.success) {
        if (result.error !== 'Payment cancelled') {
          Alert.alert(
            'Payment Failed',
            'Your order has been created but payment failed. Please contact support.',
          );
        }
        return;
      }
  ...
  const formattedAddress = [street.trim(), city.trim(), `${state.trim()} ${zipCode.trim()}`.trim()]
    .filter(Boolean)
    .join(', ');
  const summaryAddress = selectedAddress
    ? `${selectedAddress.label} • ${formattedAddress}`
    : formattedAddress;
      ...
      <View style={styles.container}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <Card style={styles.card}>
            ...
            <View style={styles.tipToggleRow}>
              ...
              <TouchableOpacity
                style={[
                  styles.tipToggleButton,
                  tipType === 'fixed' && styles.tipToggleButtonActive,
                ]}
                onPress={() => setTipType('fixed')}
              >
                <Text
          ...
          <Card style={styles.card}>
            ...
            <View style={styles.paymentInfo}>
              ...
              <View>
                ...
                <Text style={styles.paymentDesc}>Secure payment powered by Stripe</Text>
              </View>
            </View>
          </Card>
          ...
          {promoDiscount > 0 && (
            <View style={styles.promoBadge}>
              <Text style={styles.promoBadgeText}>
                Promo {promoCode ? promoCode : 'Applied'} • -{formatCurrency(promoDiscount)}
              </Text>
            </View>
          )}
          ...
          <OrderSummary
            ...
            total={total}
            deliveryAddress={summaryAddress}
