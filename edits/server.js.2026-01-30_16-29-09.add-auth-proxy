...
function saveState() {
  const state = {
    orders: orders.map((o) => ({ id: o.id, driverId: o.driverId, status: o.status })),
  ...
  try {
    ...
    if (state.orders) {
      state.orders.forEach((saved) => {
        const order = orders.find((o) => o.id === saved.id);
        if (order) {
          order.driverId = saved.driverId || order.driverId;
          order.status = saved.status || order.status;
        }
      });
    }
    if (state.scores) {
      Object.entries(state.scores).forEach(([driverId, score]) => {
        DRIVER_SCORES.set(driverId, score);
      });
    }
  } catch (err) {
    // ignore
...
function encodeFrame(data) {
  ...
  for (let i = 0; i < 8; i += 1) {
    header[9 - i] = Number((big >> BigInt(i * 8)) & 0xffn);
  }
  return Buffer.concat([header, payload]);
}
